...
% Initial conditions
% Ball position [x y z]
ball_position   = [2.2989  -12.0608 0];
% Ball velocity
ball_speed      = 40;   % Ball speed                    [m/s]
theta           = 8;   % Polar/elevation angle         [deg]
phi             = 21.7467-5;  % Azimuthal angle               [deg]
...
% % Initial conditions
% % Ball position [x y z]
% ball_position0 = [0 0 0];
% ball_position1 = [0 -15 0];
% % Ball velocity
% ball_speed = 40;   % Ball speed [m/s]
% theta0 = 10;   % Polar/elevation angle [deg]
% phi0 = -25;  % Azimuthal angle [deg]
% theta1 = 15;   % Polar/elevation angle [deg]
% phi1 = 25;  % Azimuthal angle [deg]
% % Ball initial velocity components x, y and z
% vx0 = ball_speed*cos(theta0*pi/180)*cos(phi0*pi/180);
% vy0 = ball_speed*cos(theta0*pi/180)*sin(phi0*pi/180);
% vz0 = ball_speed*sin(theta0*pi/180);
% vx1 = ball_speed*cos(theta1*pi/180)*cos(phi1*pi/180);
% vy1 = ball_speed*cos(theta1*pi/180)*sin(phi1*pi/180);
% vz1 = ball_speed*sin(theta1*pi/180);
% % Ball velocity array
% ball_velocity0 = [vx0 vy0 vz0];
% ball_velocity1 = [vx1 vy1 vz1];
% % Initial conditions
% states0 = [ball_position0 ball_velocity0];
% states1 = [ball_position1 ball_velocity1];
...
for i=1:length(tout)
    ...
    plot_field([-206.4874  199.9462   79.6781],goal_lon_pos,goal_lat_pos, ball_position)
    plot3(x(1:i),y(1:i),z(1:i),'r','LineWidth',2,'Color','r')
    plot3(x(i),y(i),z(i),'bo','MarkerFaceColor','b','MarkerSize',8)
    xlabel('x [m]')
    ylabel('y [m]')
    zlabel('z [m]')
    xticks([-10 0 10 20])
    ...
    hold on ; grid on ; axis equal
    set(gca,'FontName','Verdana','FontSize',12)
    plot_field([7.5000 -311.1862    2.5000],goal_lon_pos,goal_lat_pos, ball_position)
    ...
    hold on ; grid on ; axis equal
    set(gca,'FontName','Verdana','FontSize',12)
    plot_field([313.6862   -5.0000    2.5000],goal_lon_pos,goal_lat_pos, ball_position)
    plot3(x(1:i),y(1:i),z(1:i),'r','LineWidth',2)
    plot3(x(i),y(i),z(i),'bo','MarkerFaceColor','b','MarkerSize',4)
    xlabel('x [m]')
    ylabel('y [m]')
    ...
    hold on ; grid on ; axis equal
    set(gca,'FontName','Verdana','FontSize',12)
    plot_field([7.5000   -5.0000  308.6862],goal_lon_pos,goal_lat_pos, ball_position)
    plot3(x(1:i),y(1:i),z(1:i),'r','LineWidth',2)
    plot3(x(i),y(i),z(i),'bo','MarkerFaceColor','b','MarkerSize',4)
    xlabel('x [m]')
    ylabel('y [m]')
...
function plot_field(camera_position,goal_lon_pos,goal_lat_pos, ball_position)
    ...
    % Goal
    goal_width      = 15.32; % [m] 7.32
    goal_height     = 4.44; % [m] 2.44
    % Penalty area
    penalty_area_width  = 40.2; % [m]
    penalty_area_length = 16.5; % [m]
    % Goal area
    goal_area_width  = 23.3;    %[m]18.3
